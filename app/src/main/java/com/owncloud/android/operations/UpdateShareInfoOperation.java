...
public class UpdateShareInfoOperation extends SyncOperation {
    ...
    private void executeShareDownloadLimitOperation(OwnCloudClient client, OCShare ocShare) {
        ...
        // if download limit is -1 means current and previous download limit is same then skip updating download limit
        // updating download limit everytime resets the download counter
        if (ocShare.getShareType() == ShareType.PUBLIC_LINK && !ocShare.isFolder() && downloadLimit != -1) {
