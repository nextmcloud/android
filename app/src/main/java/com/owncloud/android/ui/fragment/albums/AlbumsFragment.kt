...
class AlbumsFragment : Fragment(), AlbumFragmentInterface, Injectable {
    ...
    override fun onResume() {
        ...
        if (requireActivity() is FileDisplayActivity) {
            ...
            if (!isSelectionMode) {
                (requireActivity() as FileDisplayActivity).highlightNavigationViewItem(R.id.nav_album)
            }
        }
    }
    ...
    override fun onItemClick(album: PhotoAlbumEntry) {
        if (isSelectionMode) {
            if (requireActivity() is AlbumsPickerActivity) {
                (requireActivity() as AlbumsPickerActivity).addFilesToAlbum(album.albumName)
