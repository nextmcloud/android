...
class FileDetailSharingFragment :
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding?.fileDetailsNestedScrollView?.setOnTouchListener { _, _ ->
            isLandscapeOrientation(requireContext()) && isSearchViewFocused
        }
    ...
    private fun scrollToSearchViewPosition(isDeviceRotated: Boolean) {
        if (isLandscapeOrientation(requireContext())) {
    ...
    private fun addExternalAndPublicShares() {
        ...
        externalShareeListAdapter?.removeAll() ?: run {
            SnackbarUtil.show(this@FileDetailSharingFragment, R.string.could_not_retrieve_shares)
            return
        }
    ...
    fun refreshSharesFromDB() {
        ...
        lifecycleScope.launch {
            ...
            withContext(Dispatchers.Main) {
                // NMC-6600 fix
                // remove shares inside Main dispatcher
