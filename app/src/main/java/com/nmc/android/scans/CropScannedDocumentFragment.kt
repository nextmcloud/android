...
class CropScannedDocumentFragment : Fragment() {
    ...
    private fun detectDocument() {
        lifecycleScope.launch {
            val initImageResult = withContext(Dispatchers.Default) {
                val scannedDocs = onDocScanListener.getScannedDocs()
                // NMC-3614 fix
                if (scannedDocs.isEmpty()) {
                    return@withContext InitImageResult(Pair(listOf(), listOf()), listOf())
                }

                originalBitmap = scannedDocs[scannedDocIndex]
                ...
                return@withContext when (result?.status) {
                    ...
                    DocumentDetectionStatus.OK_BUT_BAD_ASPECT_RATIO -> {
                        val linesPair = Pair(result.horizontalLinesNormalized, result.verticalLinesNormalized)
    ...
    private fun crop() {
        ...
        var documentImage = ImageProcessor(originalBitmap).crop(binding.cropPolygonView.polygon).processedBitmap()
        documentImage?.let {
