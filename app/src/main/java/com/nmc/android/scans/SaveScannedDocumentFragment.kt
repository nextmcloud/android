...
class SaveScannedDocumentFragment : Fragment(), CompoundButton.OnCheckedChangeListener, Injectable,
    ...
    private fun prepareRemotePath() {
        ...
        if (requireActivity() is ScanActivity) {
            val remoteFile = (requireActivity() as ScanActivity).remoteFile
    ...
    fun setRemoteFilePath(remotePath: String) {
        ...
        // NMC-3512 fix
        // here the file will not have the fileId which will fail to validate if the folder is root or any other folder
        // so we have to fetch it from storage manager
        if (remoteFilePath?.fileId == -1L) {
            remoteFilePath = (requireActivity() as ScanActivity).storageManager.getFileByDecryptedRemotePath(remotePath)
        }

        updateSaveLocationText()
    }

    private fun setRemoteFilePath(remoteFile: OCFile) {
    ...
    private fun saveScannedFiles() {
        ...
        if (TextUtils.isEmpty(fileName)) {
            SnackbarUtil.show(requireActivity(), R.string.filename_empty)
            return
        }

        if (!com.owncloud.android.lib.resources.files.FileUtils.isValidName(fileName)) {
            SnackbarUtil.show(requireActivity(), R.string.filename_forbidden_charaters_from_server)
            return
        }
        ...
        if (!binding.scanSaveWithoutTxtRecognitionPdfCheckbox.isChecked
            ...
            SnackbarUtil.show(requireActivity(), R.string.scan_save_no_file_select_toast)
            return
        }
        ...
        if (binding.scanSavePdfPasswordSwitch.isChecked && TextUtils.isEmpty(pdfPassword)) {
            SnackbarUtil.show(requireActivity(), R.string.save_scan_empty_pdf_password)
            return
        }
        ...
        if (requireActivity() is ScanActivity) {
            remoteFilePath?.let {
                (requireActivity() as ScanActivity).checkEncryption(it) { success ->
                    if (success) {
    ...
    private fun updateSaveLocationText() {
        // to upload the scan docs use remote path
        remotePath = remoteFilePath?.remotePath ?: OCFile.ROOT_PATH

        // to show file path use decrypted path
        var filePathToShow =
            remoteFilePath?.let { (requireActivity() as ScanActivity).storageManager.getDecryptedPath(it) }
                ?: OCFile.ROOT_PATH
        if (filePathToShow.isEmpty() || filePathToShow.equals(OCFile.ROOT_PATH, ignoreCase = true)) {
            filePathToShow = resources.getString(R.string.scan_save_location_root)
        }
        binding.scanSaveLocationInput.text = filePathToShow
    }
