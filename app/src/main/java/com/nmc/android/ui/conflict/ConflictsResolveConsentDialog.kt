...
class ConflictsResolveConsentDialog : DialogFragment(), Injectable {
    ...
    companion object {
        ...
        private fun getFolderConflictData(
            ...
            val existingFileData =
                ConflictFileData(existingFileTitle, existingFileTimestamp.toString(), existingFileSize)
            ...
            return ConflictDialogData(folderName, title, description, Pair(newFileData, existingFileData))
        }

        @JvmStatic
        private fun getFileConflictData(newFile: File, existingFile: OCFile, context: Context): ConflictDialogData {
            val parentFile = File(existingFile.remotePath).parentFile
