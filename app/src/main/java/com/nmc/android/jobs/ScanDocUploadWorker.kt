...
// PdfGenerator: https://github.com/doo/scanbot-sdk-example-android/tree/master/classic-components-example/pdf-generation
class ScanDocUploadWorker(
    ...
    private lateinit var pdfRenderer: PdfGenerator
    private lateinit var opticalCharacterRecognizer: OcrEngine
    ...
    private fun showNotification(pushNotificationId: Int) {
        val notificationBuilder =
            NotificationCompat.Builder(context, NotificationUtils.NOTIFICATION_CHANNEL_IMAGE_SAVE)
    ...
    private fun initScanBotSDK() {
        ...
        pdfRenderer = scanbotSDK.createPdfGenerator()
        opticalCharacterRecognizer = scanbotSDK.createOcrEngine()
    }
    ...
    private fun saveNonOCRPDFFile(fileName: String?, bitmapList: List<Bitmap>, pdfPassword: String?) {
        ...
        val outputFile = FileUtils.getPdfFileName(context, fileName)
        val isSuccess = pdfRenderer.generateFromDocument(
            pageList,
            outputFile,
        ...
        if (isSuccess) {
            outputFile?.let { file ->
                file.parent?.let { parent ->
                    val renamedFile = File(parent + OCFile.PATH_SEPARATOR + fileName + ".pdf")
            ...
            Log_OC.e(TAG, "Failed to create NonOCR PDF File")
        }
    }
    ...
    private fun getScannedPages(bitmapList: List<Bitmap>): Document {
    ...
    private fun savePDFWithOCR(fileName: String?, bitmapList: List<Bitmap>, pdfPassword: String?) {
        val document = getScannedPages(bitmapList)
        val isSuccess =
        ...
        if (isSuccess) {
            val ocrPDFFile = document.pdfUri.toFile()
            ocrPDFFile.parent?.let { parent ->
                val renamedFile = File(parent + OCFile.PATH_SEPARATOR + fileName + "_OCR.pdf")
            ...
            Log_OC.e(TAG, "Failed to create OCR PDF File")
        }
    }
    ...
    private fun saveTextFile(fileName: String?, bitmapList: List<Bitmap>) {
        for (i in bitmapList.indices) {
            ...
            val ocrResult: OcrResult = opticalCharacterRecognizer.recognizeFromBitmap(bitmap)
            val ocrPageList: List<io.scanbot.sdk.ocr.model.Page> = ocrResult.ocrPages
            if (ocrPageList.isNotEmpty()) {
                val ocrText = ocrResult.recognizedText
    ...
    private fun uploadScannedDocs(remotePathBase: String?) {
        val remotePaths = savedFiles.map {
            remotePathBase + File(it).name
        }.toTypedArray()

        FileUploadHelper.instance().uploadNewFiles(
            OCUploadLocalPathData(
                user = accountManager.user,
                localPaths = savedFiles.toTypedArray(),
                remotePaths = remotePaths,
                localBehavior = FileUploadWorker.LOCAL_BEHAVIOUR_DELETE,
                createRemoteFolder = false,  // do not create parent folder if not existent
